...
export default function FormWrapper() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-12">
      ...
      <div className="flex items-center justify-between mb-8">
        {steps.map((label, index) => (
          <div key={index} className="flex-1 text-center">
            ...
            <p className={`mt-2 text-sm ${currentStep === index ? "text-blue-600 font-semibold" : "text-gray-500"}`}>
              {label}
            </p>
          </div>
        ))}
      ...
      {/* Form Steps */}
      {currentStep === 0 && <PersonalInfoSection register={register} errors={errors} />}
      {currentStep === 1 && <EducationSection register={register} errors={errors} />}
      {currentStep === 2 && <ExperienceSection register={register} errors={errors} />}
      {currentStep === 3 && (
  <FileUploadSection
    register={register}
    setValue={setValue}
